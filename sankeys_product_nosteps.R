...
precision <- 3*1e-2 # 0.5% precision
rest_name <- "[rest]"
...
data <- data %>% 
  dplyr::mutate(code_tgt = if_else(code_tgt %in% data_tgt$code_tgt,
                                   code_tgt,
...
all_nodes <- data.frame(
  code = unique(c(data$code_src, data$code_tgt))
)

# define the number of colors
n_cols <- length(unique(all_nodes$code))
# now we get the colors
if(n_cols <= 12){
  node_colors <- data.frame(
    colors = RColorBrewer::brewer.pal(n_cols, "Set3"),
    node = unique(all_nodes$code)
  )
...
all_nodes$color <- node_colors$color[match(all_nodes$code, node_colors$node)]
...
links <- data %>%
  ...
  dplyr::left_join(all_nodes[,c("code", "index")], by = c("code_src" = "code")) %>% 
  dplyr::rename(source_index = index) %>% 
  dplyr::mutate(color = if_else(model == "fabio", "rgba(38, 166, 91, .3)", "rgba(149, 165, 166, .3)")) %>% 
  dplyr::left_join(all_nodes[,c("code", "index")], by = c("code_tgt" = "code")) %>% 
